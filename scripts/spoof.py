#!/usr/bin/env python3
"""Inject a forged IRON CURTAIN storage zone into a node to teach auth.
...
command raised the siren; against the hardened node the same attack fails
...
try:
    ...
    raise SystemExit(
        "pip install pyserial to run the CHEMICAL WARNING spoof tool"
    ) from exc
...
def _command_body(seq, zone):
    ...
    """
    head = _le32(seq) + bytes([CHEM_COMMAND_CLEAR])
...
def main():
    """Inject the forged IRON CURTAIN storage zone under the gateway address.
