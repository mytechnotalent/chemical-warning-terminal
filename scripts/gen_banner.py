#!/usr/bin/env python3
"""Generate the chemical-warning-terminal.png banner (1500x1500, stdlib).
...
def _draw_titles(banner, colors) -> None:
    ...
    None
    """
    banner.text("REMOTE CHEMICAL STORAGE WARNING TERMINAL", 4,
                colors["gold"], banner.W // 2, 90)
    banner.text("OPERATION IRON CURTAIN", 9, (0x0B, 0x10, 0x26),
                banner.W // 2 - 6, 176)
    banner.text("OPERATION IRON CURTAIN", 9, colors["white"],
                banner.W // 2, 182)
    banner.text("RP2350  PICO 2", 5, colors["cyan"], banner.W // 2, 560)
    banner.text("-=[ CHEMICAL WARNING LAB ]=-", 3, colors["dim"],
                banner.W // 2, 660)
...
def _draw_features(banner, colors) -> None:
    ...
    None
    """
    banner.text("HAZARD LEDS  SIREN  IR", 5, colors["cyan"],
                banner.W // 2, 750)
    banner.text("DHT11  WATCH  CLEAR", 4, colors["silver"],
                banner.W // 2, 900)
    banner.text("XCHACHA20-POLY1305  ARGON2ID", 4, colors["dim"],
                banner.W // 2, 1120)
    banner.text("COORDINATED BEACON", 4, colors["gold"],
                banner.W // 2, 1260)
    banner.text("INCIDENT RESPONSE", 4, colors["gold"],
                banner.W // 2, 1350)
    banner.text("-=[ OPERATION IRON CURTAIN  /  RP2350 ]=-", 3, colors["dim"],
                banner.W // 2, 1440)
...
def main() -> int:
    ...
    out = Path(__file__).resolve().parent.parent / "chemical-warning-terminal.png"
    banner.save(out)
