#!/usr/bin/env python3
"""Mesh gateway that authorizes sealed IRON CURTAIN hazard requests.
...
authenticated with the CHEM node identifier byte 0x07. An authenticated
request is decrypted to recover the requested zone, logged, and
answered with a sealed hazard command carrying a monotonic sequence
...
try:
    ...
    raise SystemExit(
        "pip install pyserial to run the CHEMICAL WARNING gateway"
    ) from exc
...
def _parse_args():
    ...
    parser.add_argument("--log", default="chem_log.csv",
...
def _field_key():
    ...
    -------
    bytes
        Thirty-two byte field key shared with the warning terminal firmware.
    """
    global _FIELD_KEY
...
def _command_body(seq, zone):
    """Build the signed body of a sealed hazard command.
    ...
    """
    head = _le32(seq) + bytes([CHEM_COMMAND_HAZARD])
    return head + zone.to_bytes(2, "little") + _state_tag(seq)


def _seal_command(seq, zone):
    """Seal a hazard command with the field key.
...
def _grant(ser, sender, zone):
    """Answer an authenticated request with a sealed hazard command.
    ...
    """
    seq = next(_SEQ)
    print(f"SIREN raise seq={seq} zone={zone}", flush=True)
...
def _open_request(record):
    """Authenticate a request and recover the guarded storage zone.
