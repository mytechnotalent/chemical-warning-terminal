#!/usr/bin/env python3
"""Simulate a real chemical warning terminal from a laptop without a Pico.
...
request and answers with a sealed hazard command. Students can validate
...
try:
    ...
    raise SystemExit(
        "pip install pyserial to run the CHEMICAL WARNING edge simulator"
    ) from exc
...
def _tick(ser, args, seq):
    """Transmit one simulated sealed warning terminal zone request.
