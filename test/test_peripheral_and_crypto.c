 ...
 *
 * DESCRIPTION:
 * Native unit tests for the RP2350 IRON CURTAIN chemical warning
 * terminal annunciator light, acknowledge button, siren servo,
...
static void assert_lamps(int red, int yellow, int green) {
    ...
    TEST_ASSERT_EQUAL_INT(yellow,
                          mock_gpio_get(CHEM_YELLOW_LED_PIN));
    TEST_ASSERT_EQUAL_INT(green,
                          mock_gpio_get(CHEM_GREEN_LED_PIN));
}
 ...
 * @return void
 */
static void assert_show(chem_led_state_t state, int red, int yellow,
                        int green) {
...
static void test_status_led_show(void) {
    ...
    assert_show(CHEM_LED_OFF, 0, 0, 0);
    assert_show(CHEM_HAZARD, 1, 0, 0);
    assert_show(CHEM_WATCH, 0, 1, 0);
    assert_show(CHEM_CLEAR, 0, 0, 1);
}
 ...
 * @return void
 */
static void test_ack_pressed(void) {
    reset_pc();
    TEST_ASSERT_TRUE(ack_init());
    gpio_put(CHEM_BUTTON_PIN, true);
    TEST_ASSERT_FALSE(ack_pressed());
    gpio_put(CHEM_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(ack_pressed());
}
 ...
 * @return void
 */
static void test_ack_consume(void) {
    reset_pc();
    ack_init();
    gpio_put(CHEM_BUTTON_PIN, true);
    TEST_ASSERT_FALSE(ack_consume_press());
    gpio_put(CHEM_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(ack_consume_press());
    TEST_ASSERT_FALSE(ack_consume_press());
}
 ...
 * @return void
 */
static void test_ack_debounce(void) {
    reset_pc();
    ack_init();
    gpio_put(CHEM_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(ack_consume_press());
 ...
 * @return void
 */
static void test_ack_debounce_elapsed(void) {
    reset_pc();
    ack_init();
    gpio_put(CHEM_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(ack_consume_press());
    ack_release();
    mock_timer_set_us(mock_timer_now_us() + CHEM_ACK_DEBOUNCE_US);
    gpio_put(CHEM_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(ack_consume_press());
}

/**
 * @brief Verify ack_reset clears the debounce state.
 *
 * @param void No parameters.
 * @return void
 */
static void test_ack_reset(void) {
    reset_pc();
    ack_init();
    gpio_put(CHEM_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(ack_consume_press());
...
static void test_servo_init(void) {
    ...
    slice = pwm_gpio_to_slice_num(CHEM_SERVO_PIN);
...
static void test_ir_init(void) {
    ...
    TEST_ASSERT_FALSE(s_mock_gpio_dirs[CHEM_IR_PIN]);
}
...
static void run_indicator_tests(void) {
    ...
    RUN_TEST(test_ack_pressed);
    RUN_TEST(test_ack_consume);
    RUN_TEST(test_ack_debounce);
    RUN_TEST(test_ack_debounce_elapsed);
    RUN_TEST(test_ack_reset);
}
